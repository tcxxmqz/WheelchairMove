...
def motor_code(EN, Dir, true_speed):
    """
    ...
    speed = int(true_speed * 219.8 * 0.75)  # 速度修正
...
def motor_full_code(FL_motor, FR_motor, BL_motor, BR_motor):
    """
...
def forward_straight_moving(true_speed):
    ...
    """
    FL_motor = motor_code(1, 0, true_speed)  # 0逆1顺
    FR_motor = motor_code(1, 1, true_speed * 1.025)  # 轮椅直线前进时由于某些原因，方向会向右偏，给右前轮一个修正速度
    BL_motor = motor_code(1, 0, true_speed)
    BR_motor = motor_code(1, 1, true_speed)
    full_code = motor_full_code(FL_motor, FR_motor, BL_motor, BR_motor)
    return full_code
